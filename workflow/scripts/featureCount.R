...
## RPKM is not good for between sample comparison, use edgeR TMM or DESeq2 "median-of-ratios"
...
# values <- mapply('/', count_table %>% summarise(across(start_with("GSM"), ~./Length*1000*1000000)), mapped_reads[,-1])
# rpkm_table <- cbind(count_table[,c(1,2)], values)
# #colnames(rpkm_table) <- mgsub(colnames(rpkm_table), c("RibosomeProfiling_", ".bam"), c("",""))

# write.table(rpkm_table, file=snakemake@output[[2]], quote=FALSE, sep="\t", col.names=TRUE, row.names=FALSE)
